...
- **Packet structure** (offsets below match the community write-up, and our own independent captures line up with these exactly for the fields we tested):
  ...
  Bytes #28, #29, and #32–#45 were checked across a full 656-packet button sweep (`full_sweep_2.txt`) and never changed value once — confirmed static/reserved, not just unchecked. Bytes #9, #10, #12, #13, #16, #27, and #31 *do* vary but aren't understood yet — see the candidate leads at the end of this section. Not covered by either source.
...
- **Byte #4 is a second, separate status byte, shared by two different buttons on different values:** `0x00` idle, `0x02` (parity bit set) while Light is being pressed (confirmed clean across 6 alternating held/released rounds), and **`0x01` = Memory Red's momentary trigger** — found by mining `full_sweep_2.txt`: byte #4 pulses to `0x01` for exactly 2 packets, then the base drives to `head=0x0720`, the identical target logged in `memory_red2.txt`/`memory_red3.txt`. Like the directional buttons, these only reflect "button currently pressed," not any persistent state. Not covered by the community write-up — this one's ours.
- **Byte #11 is a small status/flags byte with three confirmed bits:**
  ...
  - **Bit 2 (`0x04`) = "traveling to a preset," not "any motor active."** Live-tested repeatedly: firing Memory Red/Amber/Green and Flat all set it for the whole travel and clear the instant the base stops (Flat confirmed via `flatten_series.txt`: bit set in the first 2 of 8 samples while head/foot are still draining toward `0`, cleared once both land exactly on `0x0000`; Green confirmed cleanly via `memory_green2.txt`, see below). Critically, it does **not** get set by directly holding a directional button — checked against `headup_series.txt`/`footup_series.txt` (4 samples each): byte #11 stayed at base `0x20` the entire time even though the motor was clearly running (byte #2 nonzero, position visibly drifting). So this bit specifically distinguishes autonomous preset-driven movement from direct held-button movement, not motor activity in general. Manually pushing the bed by hand (no motor engaged at all) also leaves it at `0`, as expected since there's no real position sensor to detect that. The original `memory_green.txt` capture actually missed the button press entirely — 12.68 of its 15 seconds were idle before recording ended, so it only caught post-travel virtual-position drift with the moving-bit never set. A re-capture (`memory_green2.txt`) with the press correctly inside the window shows the expected `0x21 → 0x25` transition immediately on the byte #3 `0x40` trigger, resolving that gap.
  - **Bit 1 (`0x02`) = a brief "cancelling" pulse**, found in `full_sweep_2.txt`: every time a preset trigger (byte #4 `0x01`, or byte #3 `0x20`/`0x40` below) fires a *second time* while the base is still traveling from the first press, byte #11 flips from `0x25` (traveling) to `0x23` for exactly 1-2 packets, then to `0x21` as position immediately freezes. Reproduced 3/3 times in that one sweep (once for the Memory Red trigger, once each for the two byte #3 triggers). Reads as "this preset command got cancelled/interrupted," distinct from reaching the target normally (which goes straight from `0x25` to `0x21` with no `0x02` blip in between).
  - Not covered by the community write-up — all three bits are ours.
- **Byte #3 is a second bitmask, for "special function" buttons** (separate from the directional bitmask at byte #2), confirmed by holding/tapping each button and watching the byte change:
  ...
  | `0x20` | A Memory preset trigger — likely Amber, by test order (found in `full_sweep_2.txt`, same preset-travel + byte #11 signature as Memory Red's byte #4 trigger; identity not independently confirmed) |
  | `0x40` | **Memory Green**, confirmed directly (`memory_green2.txt`, 2026-07-23): fired at the exact moment Green was pressed, immediately followed by byte #11 → `0x25` and head/foot climbing together, foot settling at `0x10fc` — matching the same foot target seen for this trigger back in `full_sweep_2.txt` |

  Massage-left and Timer showed the value appearing in short repeated blips rather than one sustained block — that's the user tapping through multiple modes/durations on the same button, not the protocol itself pulsing. `0x20` was only ever seen mid-sweep, interrupted by a second press before reaching a settled target, so Amber's identity is still inferred from test order rather than independently confirmed the way Red and Green now are.
...
- **Memory Red recalls a saved head/foot position, and its trigger byte is now known** (byte #4 = `0x01`, see above). Two separate presses both drove the bed to the exact same targets — head froze at `0x0720` both times, foot climbed toward `0x0DAD` both times (froze there fully on the first attempt) — the same drift-then-freeze signature used to identify the position bytes in the first place, just now triggered autonomously by a memory recall instead of a held directional button.
- **Unconfirmed candidate bytes, found by scanning variance across the full `full_sweep_2.txt` sweep — none of these are solid enough to act on yet:**
  - **Byte #16** cycles `{0x00, 0x01, 0x02}` specifically during `center_timer.txt` (the repeated-tap timer-duration test) — plausible timer-duration index (10/20/30 min), but `timer10.txt`/`timer20.txt` don't cleanly show a 0-vs-1 split, likely because the setting persists across separate test sessions rather than resetting each time.
  - **Bytes #12/#13** always move together (identical to each other), cycling `{0x00, 0x01, 0x03, 0x06}` — possible massage pattern/intensity index, unconfirmed.
  - **Byte #10** loosely tracks the massage/timer button region (`0x01`/`0x02` near those presses vs. a default `0xFF` elsewhere) — weak signal, unconfirmed.
  - **Byte #9** toggles between `0x01`/`0x03` on its own timescale, with no correlation found to any button press — still a mystery.
  - **Bytes #27 and #31** are not a packet counter (checked — no monotonic increment across hundreds of packets); they hover in narrow bands (~`0x45`-`0x53` and ~`0x3E`-`0x40`) with small noise, more consistent with a slow analog telemetry value (temperature?) than a button flag.
...
## Next Steps
1. Confirm byte #3 `0x20` is really Memory Amber with a direct isolated test like the one that nailed down Green (`memory_green2.txt`) — currently still only inferred from test order. Preset 1/2/3 also still untested.
2. Isolate the candidate bytes #16 (timer duration), #12/#13 (massage pattern?), and #10 with dedicated single-variable tests instead of inferring from the mixed full sweep.
3. Figure out what byte #14 (and correlated #15/#22/#24/#26/#30/#46) actually represents during active massage — the sawtooth pattern suggests live intensity/PWM modulation, not confirmed.
4. Map the raw position values to actual physical angle if possible, keeping in mind they're virtual/estimated, not a real sensor reading.
5. Identify bytes #9, #27, and #31 — real variance found, no working theory yet.
6. Confirm the byte #2 directional bitmask combines correctly (e.g. Head Up + Foot Up → `0x05`), and check whether byte #3's special-function bitmask does too.
7. Determine the sync line's electrical drive type (open-drain vs. push-pull) before ever attempting to transmit.
8. Interface with Home Assistant: build a way to both listen to and *transmit* this protocol from an ESP module (8E1 framing is standard UART, so this is more tractable than we first thought when we assumed 9 raw data bits).
9. Control Bed: use the confirmed directional bitmask, position telemetry, and the community write-up's command payloads to drive real motor commands via Home Assistant.
...
- [`tools/dashboard.py`](tools/dashboard.py) is a live terminal view built on the same persistent-connection approach — polls at ~15-17Hz and redraws head/foot position (as % of the confirmed full-travel endpoints), light state, traveling/cancelling flags, and currently-active buttons. Nothing it shows is decoded differently from the rest of the tooling; it's just a live readout of the fields documented above. `python3 tools/dashboard.py <scope_ip>`.
- [`tools/web_dashboard.py`](tools/web_dashboard.py) is the same live view in a browser instead of a terminal — a small Flask app with a background polling thread, serving a page at `http://127.0.0.1:5000` that updates itself every 200ms via a `/status.json` endpoint (no page reloads). Shares its field decoding with `dashboard.py` via [`tools/bed_fields.py`](tools/bed_fields.py) so the two never drift apart. `python3 tools/web_dashboard.py <scope_ip> [--port 5000]`. Requires `flask` (see `requirements.txt`).
