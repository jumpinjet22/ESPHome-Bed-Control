...
- **Byte #11 is a small status/flags byte with three confirmed bits:**
  ...
  - **Bit 2 (`0x04`) = "traveling to a preset," not "any motor active."** Live-tested repeatedly: firing Memory Red/Green and Flat all set it for the whole travel and clear the instant the base stops (Flat confirmed via `flatten_series.txt`: bit set in the first 2 of 8 samples while head/foot are still draining toward `0`, cleared once both land exactly on `0x0000`). Critically, it does **not** get set by directly holding a directional button — checked against `headup_series.txt`/`footup_series.txt` (4 samples each): byte #11 stayed at base `0x20` the entire time even though the motor was clearly running (byte #2 nonzero, position visibly drifting). So this bit specifically distinguishes autonomous preset-driven movement from direct held-button movement, not motor activity in general. Manually pushing the bed by hand (no motor engaged at all) also leaves it at `0`, as expected since there's no real position sensor to detect that. **Caveat:** the saved `memory_green.txt` capture does *not* itself demonstrate this — its 178 samples show byte #11 sitting constant at `0x21` while head slowly drifts on its own (18 distinct values, no motor engaged) — that capture window almost certainly started after the real button press+travel had already finished, so it only recorded post-travel virtual-position drift. The bit-sets-during-Green-travel claim currently rests on live real-time checks made during the session, not on that file; worth a clean re-capture.
...
- **Byte #3 is a second bitmask, for "special function" buttons** (separate from the directional bitmask at byte #2), confirmed by holding/tapping each button and watching the byte change:
  ...
  | `0x40` | A second Memory preset trigger — likely Green, by test order (same file, same signature; identity not independently confirmed) |

  Massage-left and Timer showed the value appearing in short repeated blips rather than one sustained block — that's the user tapping through multiple modes/durations on the same button, not the protocol itself pulsing. `0x20`/`0x40` were only ever seen mid-sweep, each interrupted by a second press before reaching a settled target, so which is Amber and which is Green is inferred from test order, not confirmed by a matching recalled position the way Memory Red was.
...
## Next Steps
1. Nail down which of byte #3's `0x20`/`0x40` is Memory Amber and which is Memory Green (currently only inferred from test order) — isolated single-button tests like the massage-left/right ones got, each run to a settled target rather than interrupted mid-travel. Preset 1/2/3 also still untested.
2. Get a clean, uninterrupted Memory Green capture — the existing `memory_green.txt` only recorded post-travel drift, not the actual trigger + travel (see byte #11 caveat above).
3. Isolate the candidate bytes #16 (timer duration), #12/#13 (massage pattern?), and #10 with dedicated single-variable tests instead of inferring from the mixed full sweep.
...
6. Identify bytes #9, #27, and #31 — real variance found, no working theory yet.
